...
router.get('/available-slots', async (req, res) => {
  try {
    ...
    const availableSlots = await googleCalendar.getAvailableSlots(date, parseInt(duration));
...
router.post('/', auth, [
  ...
  body('meetingType').optional().isIn(['consultation', 'follow-up', 'project-discussion']),
  ...
  try {
    ...
    if (duration < 30 || duration > 180) {
      return res.status(400).json({ error: 'Meeting duration must be between 30 and 180 minutes' });
    }
    ...
    const booking = new Booking({
      ...
      description,
      amount: parseFloat(process.env.MEETING_FEE) || 50.00
    });
...
router.get('/:id', auth, async (req, res) => {
  try {
    ...
    if (booking.client._id.toString() !== req.user.userId && req.user.role !== 'admin') {
...
router.put('/:id/cancel', auth, [
  ...
  try {
    ...
    const hoursUntilMeeting = (booking.startTime - new Date()) / (1000 * 60 * 60);
