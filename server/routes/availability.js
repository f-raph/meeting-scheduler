...
router.get("/", async (req, res) => {
  try {
    ...
    // Multi-tenant: prefer tenant from middleware, fallback to explicit adminId
    const tenantId = req.tenantAdminId || adminId;
    if (tenantId) {
      query.ownerAdmin = tenantId;
    }
    ...
    const availability = await Availability.find(query).sort({
      dayOfWeek: 1,
      startTime: 1,
    });
...
router.get("/day/:dayOfWeek", async (req, res) => {
  try {
    ...
    if (req.tenantAdminId) {
      baseQuery.ownerAdmin = req.tenantAdminId;
    }

    const availability = await Availability.find(baseQuery).sort({
      startTime: 1,
    });
...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      // Check for conflicts (scoped to current admin)
      const query = {
      ...
      if (conflictingSlot) {
        return res
          .status(400)
          .json({ error: "Time slot conflicts with existing availability" });
      }
