...
// Initialize payment
router.post('/initialize-payment', auth, [
  ...
  try {
    ...
    switch (event.event) {
      case 'charge.success':
        ...
        try {
          const booking = await Booking.findOne({ paymentIntentId: paymentData.reference })
            .populate('client', 'firstName lastName email');
      ...
      case 'charge.failed':
        ...
        try {
          const booking = await Booking.findOne({ paymentIntentId: failedPayment.reference });
          if (booking) {
